...
public interface IBookRepository extends JpaRepository<Book,Integer> {
    @Query(value = "select * from book join category on category.id = book.category_id" +
            " where `status` = 0 and book.name like :name or price like :name or author like :name and category.name like :category" +
            " order by book.id desc",
    countQuery ="select count(*) from book join category on category.id = book.category_id" +
            " where `status` = 0  and book.name like :name or price like :name or author like :name and category.name like :category order by book.id desc",nativeQuery = true)
